...
Author URI: http://codex.pierros.fr/
Text Domain: kodex
...
class Kodex_Posts_Likes{
	...
	public function get_options(){
		$this->default = array(
			...
			'hide_counter_0' => array(
				'label' => __("Hide the counter if the number is zero", 'kodex'),
				'value' => true,
				'type'  => 'checkbox'
			),
			'post_types' => array(
	...
	private function post_types_maybe(){
		...
		$types = array();
		foreach($wp_post_types as $i=>$v){
			if( !in_array($i, array('revision','nav_menu_item','acf')) ){
				$types[] = $i;
			}
		}
		return $types;
						...
						<tr class="<?php echo $class; ?>">
							...
							<td>
								...
								<?php elseif($object->type=='array'):
									foreach($object->maybe as $i): ?>
										<label>
											<input type="checkbox" name="<?php echo $this->plugin_slug; ?>[<?php echo $k; ?>][]" value="<?php echo esc_attr($i); ?>" <?php echo(in_array($i, $value))?'checked':''; ?>>
											<?php echo $i; ?>
										</label><br>
									<?php endforeach; ?>
	...
	public function admin_scripts(){
	    ...
	    wp_localize_script('kodex_posts_likes', 'kodex_posts_likes', array('settings'=>$this->plugin_settings));
	...
	private function buttons($post_id){
		...
		$html .= '<button type="button" class="kodex_button kodex_like_button'.$like_active.'" data-id="'.$post_id.'" data-action="like">
			...
			<span class="text">'.$this->get_option('like_text').'</span>';
		...
		if( $this->get_option('show_dislike') ){
			...
			$html .= '<button type="button" class="kodex_button kodex_dislike_button'.$dislike_active.'" data-id="'.$post_id.'" data-action="dislike">
				<span class="icon"></span>
				<span class="text">'.$this->get_option('dislike_text').'</span>';
	...
	public function ajax(){
		...
		$test = wp_verify_nonce($_REQUEST['nonce'], 'kodex_posts_likes');
		if($test){
